const Collection = require("./src/Collection.js")
    , Debugger = require("./src/utils/Debugger.js")
            ...
            axios.get("https://raw.githubusercontent.com/" + o.data.repository.url.split("/")[3] + "/" + o.data.repository.url.split("/")[4].split(".git")
                ...
                .then(o => {
                    o.data.version.split(".")
                        ...
                        .join("") && Debugger.log("\x1b[33mNEW VERSION AVAILABLE! \x1b[32mYou are currently using version \x1b[35mv" + Package.version + "\x1b[32m. New version \x1b[35mv" + o.data.version + "\x1b[32m is available. You can update it using this command: \x1b[35mnpm install " + o.data.name)
                })
                .catch(() => {
